...
<div class="min-h-screen bg-gray-50 md:bg-white md:min-h-0 pb-20 md:pb-0">
    ...
    <div class="container mx-auto px-0 md:px-0 md:border md:border-t-0 md:border-gray-100 md:rounded-b-xl md:shadow-lg md:mb-8 bg-white">
        <form action="{{ route('admin.announcements.update', $announcement) }}" method="POST" enctype="multipart/form-data" id="editForm">
            @csrf
            @method('PUT')
            ...
            <div class="p-4 md:p-8 space-y-6">
                ...
                <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-3 flex items-center gap-4">
                    <img src="{{ $announcement->image_url }}" alt="Actual" class="h-20 w-20 object-cover rounded-lg bg-gray-100 flex-shrink-0">
                    <div>
                ...
                <div>
                    ...
                    <textarea name="short_description" required rows="2" 
                              class="block w-full rounded-lg border-gray-300 bg-gray-50 border focus:bg-white focus:border-button focus:ring-button transition-colors p-3">{{ old('short_description', $announcement->short_description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea name="full_description" required rows="6" 
                              class="block w-full rounded-lg border-gray-300 bg-gray-50 border focus:bg-white focus:border-button focus:ring-button transition-colors p-3">{{ old('full_description', $announcement->full_description) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 pt-2">
                    <div>
                        ...
                        <input type="number" name="order" value="{{ old('order', $announcement->order) }}" class="block w-full rounded-lg border-gray-300 bg-gray-50 p-2.5">
                    </div>
                    <div class="flex items-center justify-center bg-gray-50 rounded-lg border border-gray-200 mt-6 md:mt-0 h-[46px] self-end">
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ $announcement->is_active ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-button focus:ring-button">
