                        ...
                        <div class="announcement-card bg-white rounded-3xl overflow-hidden shadow-sm text-center flex flex-col h-full border border-gray-100 w-full group hover:shadow-xl transition-all duration-300">
                            <div class="w-full aspect-video bg-gray-50 flex items-center justify-center overflow-hidden relative">
                                @if($announcement->image_url)
                                    <img src="{{ $announcement->image_url }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700">
                                @else
    ...
    <div class="container max-w-7xl mx-auto px-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="schedule-card bg-white rounded-3xl shadow-lg border border-gray-100 flex flex-col h-auto overflow-hidden">
                <div class="schedule-header p-6 flex flex-row md:flex-col items-center justify-between md:justify-center cursor-pointer md:cursor-default group relative z-10">
                    <div class="flex items-center gap-4 md:flex-col md:gap-6">
                        <div class="w-14 h-14 bg-nav-footer rounded-2xl flex items-center justify-center group-hover:bg-button transition-colors"><img src="{{ asset('img/icono_misas.png') }}" class="h-8 w-auto"></div>
                ...
                <div class="schedule-content hidden md:block px-6 pb-8 pt-0 md:pt-2 bg-white border-t md:border-t-0 border-gray-50">
                    <div class="space-y-5 text-text-dark text-center">
                        ...
                        <div><span class="block font-black text-gray-400 text-[10px] uppercase tracking-widest mb-1">Domingo</span><p class="text-gray-800 font-bold text-[13px]">08:00 | 09:30 | 11:00<br>12:30 | 18:00 | 19:30<br>20:30</p></div>
                    </div>
                </div>
            </div>
            {{-- Confesiones --}}
            <div class="schedule-card bg-white rounded-3xl shadow-lg border border-gray-100 flex flex-col h-auto overflow-hidden">
                <div class="schedule-header p-6 flex flex-row md:flex-col items-center justify-between md:justify-center cursor-pointer md:cursor-default group relative z-10">
                    <div class="flex items-center gap-4 md:flex-col md:gap-6">
                        <div class="w-14 h-14 bg-nav-footer rounded-2xl flex items-center justify-center group-hover:bg-button transition-colors"><img src="{{ asset('img/icono_confesiones.png') }}" class="h-8 w-auto"></div>
            ...
            <div class="schedule-card bg-white rounded-3xl shadow-lg border border-gray-100 flex flex-col h-auto overflow-hidden">
                <div class="schedule-header p-6 flex flex-row md:flex-col items-center justify-between md:justify-center cursor-pointer md:cursor-default group relative z-10">
                    <div class="flex items-center gap-4 md:flex-col md:gap-6">
                        <div class="w-14 h-14 bg-nav-footer rounded-2xl flex items-center justify-center group-hover:bg-button transition-colors"><img src="{{ asset('img/icono_secretaria.png') }}" class="h-8 w-auto"></div>
            ...
            <div class="schedule-card bg-white rounded-3xl shadow-lg border border-gray-100 flex flex-col h-auto overflow-hidden">
                <div class="schedule-header p-6 flex flex-row md:flex-col items-center justify-between md:justify-center cursor-pointer md:cursor-default group relative z-10">
                    <div class="flex items-center gap-4 md:flex-col md:gap-6">
                        <div class="w-14 h-14 bg-nav-footer rounded-2xl flex items-center justify-center group-hover:bg-button transition-colors"><img src="{{ asset('img/icono_donaciones.png') }}" class="h-8 w-auto"></div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (track && container) {
        ...
        track.addEventListener('touchend', (e) => {
            ...
            let threshold = 50; // Distancia mínima para detectar swipe

            if (Math.abs(diffX) > threshold) {
                let show = window.innerWidth >= 1024 ? 3 : (window.innerWidth >= 768 ? 2 : 1);
                if (diffX > 0) { // Siguiente (Swipe izquierda)
                    if (index < items.length - show) { index++; updateCarousel(); }
                } else { // Anterior (Swipe derecha)
                    if (index > 0) { index--; updateCarousel(); }
                }
            }
        }, {passive: true});
...
<style>
    ...
    .bg-gospel-bg { background-color: #f0f9ff; }
</style>
@endsection