...
<div class="bg-gray-50 min-h-screen py-8 md:py-12">
    <div class="container max-w-7xl mx-auto px-4 text-center md:text-left">
        ...
        <div class="mb-10">
            <h1 class="text-3xl md:text-4xl font-black text-gray-900 uppercase tracking-tighter">Bienvenido, {{ Auth::user()->name }}</h1>
            ...
            <div class="lg:col-span-2 space-y-8 text-left">
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        @endphp
                        @forelse($userGroups as $role)
                            @php $slug = str_replace('admin_', '', $role->name); $isAdmin = Auth::user()->isAdminOfGroup($slug); @endphp
                            <a href="{{ route('grupos.dashboard', $slug) }}" class="flex items-center p-4 bg-gray-50 rounded-2xl border-2 border-transparent hover:border-button/30 hover:bg-white transition-all group">
                                <div class="w-12 h-12 rounded-xl bg-blue-900 text-white flex items-center justify-center font-black uppercase text-lg shadow-sm group-hover:scale-105 transition-transform">{{ substr($slug, 0, 1) }}</div>
                                <div class="ml-4 min-w-0 flex-1">
                                    <span class="block font-bold text-gray-800 uppercase text-xs truncate">{{ str_replace('_', ' ', $slug) }}</span>
                                    <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{{ $isAdmin ? 'Gestionar' : 'Materiales' }} →</span>
                                </div>
                            </a>
                        @empty
                            <div class="col-span-full py-12 text-center border-2 border-dashed border-gray-200 rounded-3xl">
                                ...
                                <a href="{{ route('grupos.index') }}" class="text-button font-black uppercase text-[10px] tracking-widest mt-4 inline-block hover:underline">Explorar grupos →</a>
                            </div>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    @if($isMember)
                    <a href="{{ route('diario.index') }}" class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all flex items-center gap-4">
                        <div class="p-3 bg-blue-50 rounded-2xl"><img src="{{ asset('img/icono_biblia.png') }}" class="w-8 h-8"></div>
                        ...
                        <div class="announcement-wrapper flex-shrink-0 px-3 box-border flex justify-center w-full md:w-1/2 lg:w-1/3">
                            <div class="announcement-card bg-white rounded-3xl overflow-hidden shadow-sm text-center flex flex-col h-full border border-gray-100 w-full group hover:shadow-xl transition-all duration-300">
                                <div class="w-full aspect-video bg-gray-50 flex items-center justify-center overflow-hidden relative">
                                    @if($announcement->image_url) <img src="{{ $announcement->image_url }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"> @endif
        ...
        <div class="max-w-4xl mx-auto px-6">
            <div class="gospel-content">
                ...
                @if($evangelioHoy && $evangelioHoy->contenido)
                    ...
                    <p class="gospel-reference text-gray-700 font-black uppercase tracking-widest text-sm text-right">— {{ $evangelioHoy->referencia }}</p>
                @else
                    <p class="gospel-text italic text-lg md:text-xl mb-6 leading-loose text-gray-700 font-medium">
        ...
        <section class="py-20 pb-20">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-left">
                ...
                <div class="schedule-card bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col h-auto overflow-hidden">
                    <div class="schedule-header p-8 flex flex-row md:flex-col items-center justify-between md:justify-center cursor-pointer md:cursor-default transition-colors group bg-gray-50 border-b border-gray-100 hover:bg-white">
                        <div class="flex items-center gap-4 md:flex-col md:gap-6">
                            <div class="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform"><img src="{{ asset('img/icono_misas.png') }}" class="h-9 w-auto"></div>
                ...
                <div class="schedule-card bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col h-auto overflow-hidden">
                    <div class="schedule-header p-8 flex flex-row md:flex-col items-center justify-between md:justify-center cursor-pointer md:cursor-default transition-colors group bg-gray-50 border-b border-gray-100 hover:bg-white">
                        <div class="flex items-center gap-4 md:flex-col md:gap-6">
                            <div class="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform"><img src="{{ asset('img/icono_confesiones.png') }}" class="h-9 w-auto"></div>
                ...
                <div class="schedule-card bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col h-auto overflow-hidden">
                    <div class="schedule-header p-8 flex flex-row md:flex-col items-center justify-between md:justify-center cursor-pointer md:cursor-default transition-colors group bg-gray-50 border-b border-gray-100 hover:bg-white">
                        <div class="flex items-center gap-4 md:flex-col md:gap-6">
                            <div class="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform"><img src="{{ asset('img/icono_secretaria.png') }}" class="h-9 w-auto"></div>
                ...
                <div class="schedule-card bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col h-auto overflow-hidden">
                    <div class="schedule-header p-8 flex flex-row md:flex-col items-center justify-between md:justify-center cursor-pointer md:cursor-default transition-colors group bg-gray-50 border-b border-gray-100 hover:bg-white">
                        <div class="flex items-center gap-4 md:flex-col md:gap-6">
                            <div class="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform"><img src="{{ asset('img/icono_donaciones.png') }}" class="h-9 w-auto"></div>
    ...
    @if(isset($announcements))
        @foreach($announcements as $announcement)
        <div id="modal-{{ $announcement->id }}" class="modal hidden fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
            <div class="bg-white rounded-3xl max-w-4xl max-h-[90vh] shadow-2xl flex flex-col overflow-hidden relative animate-fade-in">
                ...
                <div class="p-6 md:p-10 overflow-y-auto custom-scrollbar">
                    @if($announcement->image_url)
                        <div class="rounded-3xl overflow-hidden mb-8 shadow-inner bg-gray-50"><img src="{{ $announcement->image_url }}" class="w-full h-auto max-h-[45vh] object-contain mx-auto"></div>
                    @endif
                    <h3 class="text-2xl md:text-4xl font-black text-gray-900 mb-6 uppercase tracking-tighter border-b-4 border-button pb-4 pr-16">{{ $announcement->title }}</h3>
                    <div class="text-gray-700 mb-10 whitespace-pre-line leading-relaxed text-base font-medium text-justify">{!! nl2br(e($announcement->full_description)) !!}</div>
                </div>
            </div>
        </div>
        @endforeach
