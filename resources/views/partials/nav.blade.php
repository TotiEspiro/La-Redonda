    ...
    <div class="container max-w-7xl mx-auto px-4">
        <div class="flex justify-between items-center">      
            ...
            <div class="flex items-center space-x-2 md:space-x-6">
                ...
                <a href="{{ url('/donaciones') }}" class="hidden md:inline-block bg-button text-white px-6 py-2 rounded-lg font-bold text-base hover:bg-blue-900 transition-all shadow-sm">Donaciones</a>
                    ...
                    <div id="desktopMenu" class="hidden absolute right-0 mt-3 w-72 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden slide-up-fade">
                        <div class="max-h-[85vh] overflow-y-auto custom-scrollbar">
                            ...
                            <div class="p-3 bg-yellow-400">
                                <a href="{{ $isRestricted ? route('verification.notice') : route('admin.dashboard') }}" class="flex items-center justify-center w-full py-3 bg-white/20 hover:bg-white/30 border border-white/30 rounded-xl text-black text-[11px] font-black uppercase tracking-widest transition-all">
                            ...
                            <div class="py-2 border-b border-gray-50">
                                <a href="{{ $isRestricted ? route('verification.notice') : $inicioUrl }}" class="block px-5 py-3 text-xs font-bold text-gray-700 hover:bg-blue-50 transition uppercase">Inicio</a>
                            ...
                            @auth
                                ...
                                <div class="bg-gray-50/30">
                                    @if($hasAccessToDiario)
                                        <a href="{{ $isRestricted ? route('verification.notice') : route('diario.index') }}" class="flex items-center px-5 py-4 text-xs font-bold border-b uppercase border-gray-100 text-gray-700 hover:bg-blue-50 transition">
                                            ...
                                            <div id="pcComuSub" class="accordion-content bg-gray-50/50 px-3">
                                                @foreach($memberGroups as $slug)
                                                    ...
                                                    <a href="{{ $targetRoute }}" class="block p-2 text-[9px] font-bold border border-gray-100 rounded bg-white hover:bg-button hover:text-white text-gray-600 truncate uppercase mb-1">
                                                        @if(!$isRestricted && $groupData && $groupData->group_password && !session('group_unlocked_' . $slug))
                                                            🔒 {{ str_replace('_', ' ', $slug) }}
                                                        @else
                                                            {{ str_replace('_', ' ', $slug) }}
                                                        @endif
                                        ...
                                        <div class="border-b border-gray-50">
                                            ...
                                            <div id="pcGestionSub" class="accordion-content bg-gray-50/50 px-3">
                                                @foreach($managedGroups as $g)
                                                    <a href="{{ $isRestricted ? route('verification.notice') : route('grupos.dashboard', str_replace('admin_', '',  $g->name)) }}" class="block p-2 text-[9px] font-bold border border-gray-100 rounded bg-white hover:bg-button hover:text-white text-gray-600 truncate uppercase mb-1">{{ $g->display }}</a>
                                                @endforeach
                                            </div>
                                        </div>
                                    @endif

                                    <a href="{{ $isRestricted ? route('verification.notice') : route('profile.show') }}" class="flex items-center px-5 py-4 text-xs font-bold text-gray-700 hover:bg-blue-50 transition border-b border-gray-50 uppercase">
...
<div class="md:hidden">
    <nav class="fixed bottom-0 left-0 w-full bg-nav-footer border-t-2 border-sky-400 z-50 h-16 shadow-lg">
        <div class="flex justify-around items-center h-full">
            <a href="{{ $isRestricted ? route('verification.notice') : $inicioUrl }}" class="flex flex-col items-center justify-center w-full h-full text-gray-600"><img src="{{ asset('img/icono_inicio.png') }}" class="h-6 w-6 mb-1"><span class="text-[9px] font-black uppercase">Inicio</span></a>
    ...
    <div id="bottomMenuCard" class="hidden fixed bottom-20 right-4 left-4 bg-white rounded-3xl shadow-2xl border border-gray-100 z-50 slide-up-fade overflow-hidden">
        ...
        <div class="max-h-[60vh] overflow-y-auto p-4 space-y-4 bg-gray-50/30">
            ...
            <a href="{{ $isRestricted ? route('verification.notice') : route('admin.dashboard') }}" class="flex items-center w-full px-5 py-4 bg-yellow-400 text-black rounded-2xl shadow-md transition-transform active:scale-95">
                <img src="{{ asset('img/icono_admin.png') }}" class="w-6 h-6 mr-4"> <span class="text-xs font-black uppercase">Panel de Administrador</span>
            </a>
            @endif

            {{-- MIS COMUNIDADES MÓVIL --}}
            @if($memberGroups->isNotEmpty())
            ...
            @if($managedGroups->isNotEmpty())
                <div class="bg-white border border-blue-100 rounded-2xl overflow-hidden">
                    ...
                    <div id="mobSuperSub" class="accordion-content bg-white/50">
                        <div class="grid grid-cols-2 gap-2 p-2">
                            @foreach($managedGroups as $g)
                                <a href="{{ $isRestricted ? route('verification.notice') : route('grupos.dashboard', str_replace('admin_', '', $g->name)) }}" class="grid-menu-item bg-white border border-gray-100 rounded-xl text-[8px] font-bold text-gray-500 uppercase shadow-sm">{{ $g->display }}</a>
                            @endforeach
                        </div>
                    </div>
                </div>
            ...
            @if($hasAccessToDiario)
                <a href="{{ $isRestricted ? route('verification.notice') : route('diario.index') }}" class="flex items-center w-full px-5 py-4 bg-white border border-gray-100 rounded-2xl text-gray-700 shadow-sm"><img src="{{ asset('img/icono_biblia.png') }}" class="w-6 h-6 mr-4"><span class="text-xs font-black uppercase">Diario de La Redonda</span></a>
            @endif
            
            <a href="{{ $isRestricted ? route('verification.notice') : route('profile.show') }}" class="flex items-center w-full px-5 py-4 bg-white border border-gray-100 rounded-2xl text-gray-700 shadow-sm"><img src="{{ asset('img/icono_perfil.png') }}" class="h-6 h-6 mr-4"><span class="text-xs font-black uppercase">Mi Perfil</span></a>
