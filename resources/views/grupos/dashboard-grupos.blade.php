...
<div class="bg-gray-50 min-h-screen pb-20">
    ...
    <div class="bg-button text-white pt-10 pb-20 shadow-lg relative overflow-hidden">
        ...
        <div class="container max-w-7xl mx-auto px-4 relative z-10">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    <h1 class="text-4xl md:text-5xl font-black uppercase tracking-tighter leading-none mb-2">{{ $groupName }}</h1>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <a href="{{ route('grupos.materials', $group->category) }}" class="bg-blue-900 text-white border border-white/20 px-6 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest backdrop-blur-sm hover:scale-105 active:scale-95  transition-all">
                        Ver Biblioteca
                    </a>
                </div>
            </div>
    ...
    <div class="container max-w-7xl mx-auto px-4 -mt-12 relative z-20">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            {{-- LADO IZQUIERDO: SOLICITUDES Y RECURSOS --}}
            <div class="lg:col-span-8 space-y-8">
                ...
                <div class="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex items-center justify-between p-6 bg-gray-50 rounded-2xl mb-3 border-2 border-transparent hover:border-button/20 transition-all">
                            <div>
                                <p class="text-sm font-black text-gray-800 uppercase tracking-tight">{{ $req->name }}</p>
                                <p class="text-[11px] text-gray-400 font-medium">{{ $req->email }}</p>
                            </div>
                            <div class="flex gap-3">
                                <form action="{{ route('grupos.handle-request', $req->id) }}" method="POST">
                ...
                {{-- Gestión de Materiales Recientes --}}
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="p-5 border border-gray-100 rounded-3xl hover:bg-blue-50/50 transition-all group flex items-center justify-between">
                            <div class="flex items-center gap-4 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <p class="text-xs font-black text-gray-700 truncate uppercase tracking-tight">{{ $m->title }}</p>
                                    <p class="text-[9px] text-gray-400 font-bold uppercase">{{ $m->created_at->format('d/m/Y') }}</p>
                                </div>
                            </div>
                            <div class="flex gap-2">
                                <button onclick="confirmDeleteResource({{ $m->id }}, '{{ $m->title }}')" class="p-2 text-red-300 hover:text-red-500 hover:bg-white rounded-lg transition-colors"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            {{-- LADO DERECHO: MIEMBROS Y MÉTRICAS --}}
            <div class="lg:col-span-4 space-y-8">
                
                {{-- ESTADO OPERATIVO (AJUSTADO: Iconos a la derecha) --}}
                <div class="bg-white rounded-3xl shadow-xl border border-gray-100 p-8">
                    ...
                    <div class="p-3 max-h-[400px] overflow-y-auto custom-scrollbar">
                        ...
                        <div class="flex items-center justify-between p-4 hover:bg-gray-50 rounded-2xl transition-all group">
                            <div class="flex items-center gap-4 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <p class="text-xs font-black text-gray-800 uppercase tracking-tight truncate">{{ $member->name }}</p>
                                    @if($member->isAdminOfGroup($group->category)) <span class="text-[7px] font-black text-button bg-blue-50 px-1 py-0.5 rounded uppercase border border-blue-100">Coordinador</span> @endif
                                </div>
                            </div>
                            @if(Auth::id() !== $member->id)
                            <button onclick="confirmRemoveMember({{ $member->id }}, '{{ $member->name }}')" class="text-gray-300 hover:text-red-500 transition-colors p-2"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
                            @endif
                        </div>
                        @endforeach
                    </div>
...
<script>
    ...
    document.getElementById('uploadForm')?.addEventListener('submit', async function(e) {
        ...
        try {
            const res = await fetch("{{ route('grupos.upload-material', $groupRole) }}", {
                method: 'POST',
    ...
    document.getElementById('btnFinalConfirm')?.addEventListener('click', async function() {
        ...
        try {
            ...
            if(currentActionType === 'resource') {
                ...
                url = `/grupos/panel/{{ $group->category }}/members/${currentActionId}`;
            }
