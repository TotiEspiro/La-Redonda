                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-300 material-card flex flex-col"
                    ...
                    <div class="p-5 flex-1 flex flex-col">
                        <div class="flex justify-between items-start mb-4">
                            <img src="{{ asset($material->file_icon) }}" class="w-10 h-10 object-contain" onerror="this.src='{{ asset('img/icono_docs.png') }}'">
                        ...
                        <h3 class="font-bold text-gray-900 mb-2 line-clamp-2 text-lg uppercase tracking-tight">{{ $material->title }}</h3>
                        <p class="text-sm text-gray-500 mb-4 line-clamp-2 flex-1">{{ $material->description ?? 'Sin descripción disponible.' }}</p>
                    ...
                    <div class="bg-gray-50 px-5 py-3 border-t border-gray-200 flex gap-2">
                        <a href="{{ route('groups.download', $material->id) }}" class="flex-1 bg-white text-button border border-button border-opacity-30 text-center py-2 rounded-lg text-xs font-black uppercase hover:bg-button hover:text-white transition-all shadow-sm">Descargar</a>
                        @if($material->can_preview)
                        <button onclick="previewMaterial({{ $material->id }}, '{{ $material->file_type }}', '{{ route('groups.view', $material->id) }}', '{{ $material->title }}')" class="px-4 py-2 bg-blue-100 text-blue-600 rounded-lg text-xs font-black uppercase hover:bg-blue-200 transition-all">Abrir</button>
                        @endif
                    </div>
                </div>
            @endforeach
...
{{-- MODAL DE VISTA PREVIA (CON PANTALLA COMPLETA) --}}
