<?php if(auth()->guard()->check()): ?>
    ...
    <?php elseif($noAge): ?>
        <a href="<?php echo e(route('profile.edit')); ?>" class="block w-full py-4 bg-yellow-50 text-yellow-700 text-center rounded-2xl font-black text-[10px] uppercase tracking-widest border-2 border-dashed border-yellow-200">
    ...
    <?php else: ?>
        <button onclick="openJoinModal('<?php echo e($slug); ?>', '<?php echo e($groupInfo->name ?? str_replace('_', ' ', $slug)); ?>')" 
        ...
        <div id="modal-join-<?php echo e($slug); ?>" class="fixed inset-0 z-[150] hidden items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fade-in">
            <div class="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm p-8 text-center animate-slide-up">
                ...
                <p class="text-gray-500 text-sm leading-relaxed mb-8">
                    Participar en <strong><?php echo e($groupInfo->name ?? str_replace('_', ' ', $slug)); ?></strong> requiere asistencia y compromiso. ¿Estás seguro de que puedes participar de sus actividades?
                </p>
                <div class="flex flex-col gap-3">
                    <form action="<?php echo e(route('grupos.send-request', $slug)); ?>" method="POST">
                        <?php echo csrf_field(); ?>
                    ...
                    <button onclick="closeJoinModal('<?php echo e($slug); ?>')" class="w-full py-4 text-gray-400 font-bold uppercase text-[10px] tracking-widest">Aún no</button>
                </div>
            </div>
        </div>
    <?php endif; ?>
<?php else: ?>
    <a href="<?php echo e(route('login')); ?>" class="block w-full py-4 bg-gray-50 text-gray-400 text-center rounded-2xl font-black text-[10px] uppercase tracking-widest border border-gray-100 hover:bg-gray-100">
...
</script><?php /**PATH C:\laragon\www\copia_laredo\La-Redonda\resources\views/partials/group-join-button.blade.php ENDPATH**/ ?>