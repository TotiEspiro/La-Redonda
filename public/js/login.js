/**
 * Lógica de pantalla de carga y utilidades de login/register
 */
...
function showProgressLoading() {
    ...
    if (!loadingScreen || !loadingProgress || !loadingPercent) return;
...
document.addEventListener('DOMContentLoaded', function() {
    // 1. Submit de formularios
    const loginForm = document.getElementById('loginForm');
    const registerForm = document.getElementById('registerForm');
    
    if (loginForm) loginForm.addEventListener('submit', showProgressLoading);
    if (registerForm) registerForm.addEventListener('submit', showProgressLoading);

    // 2. Click en botones sociales (Google/Facebook)
    const socialButtons = document.querySelectorAll('.social-btn');
    socialButtons.forEach(btn => {
        btn.addEventListener('click', function(e) {
            // Permitimos la redirección pero mostramos el loading
            showProgressLoading();
        });
    });
});